<?php

class CRP_Migration {

    public function __construct()
    {
        add_action( 'admin_init', array( $this, 'migrate_if_needed' ) );
    }

    public function migrate_if_needed()
    {
        // Get current migrated to version
        $migrate_version = get_option( 'crp_migrate_version', false );

        if( !$migrate_version ) {
            $notices = false;
            $migrate_version = '0.0.1';
        } else {
            $notices = true;
        }

        $migrate_special = '';
        if( isset( $_GET['crp_migrate'] ) ) {
            $migrate_special = $_GET['crp_migrate'];
        }

//        if( $migrate_version < '1.2' ) require_once( CustomRelatedPosts::get()->coreDir . '/helpers/migration/1_2_active_colors.php');
    }
}